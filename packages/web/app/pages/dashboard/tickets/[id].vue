...
<script setup lang="ts">
  ...
  const slaStatus = computed(() => {
    ...
    if (
      ticket.value.priority === 'URGENT' &&
      ['OPEN', 'IN_PROGRESS'].includes(ticket.value.status)
    ) {
      return {
        label: 'At risk',
...
<template>
  <div class="animate-fade-in">
    ...
    <div class="flex items-center gap-3 mb-5">
      <NuxtLink to="/dashboard/tickets" class="text-white/40 hover:text-white/70 transition-colors">
      ...
      <h2 class="text-lg font-semibold text-white truncate">
        {{
          ticketsStore.isLoadingDetail
            ? 'Loading ticket...'
            : (ticket?.subject ?? 'Ticket not found')
        }}
      </h2>
    </div>
    ...
    <div v-else class="grid lg:grid-cols-3 gap-5">
        ...
        <UiGlassCard padding="none">
          <div class="px-5 py-4 border-b border-white/10 flex items-center justify-between">
            ...
            <span class="text-white/40 text-xs"
              >{{ ticket.messages?.length || (ticket.body ? 1 : 0) }} messages</span
            >
          </div>
          ...
          <div class="px-5 py-4 border-t border-white/10">
                  ...
                  <Transition name="fade">
                    ...
                    >
                      ...
                      <div class="overflow-y-auto max-h-52 divide-y divide-white/[0.04]">
                        ...
                        >
                          <div class="flex items-center gap-2">
                            <span class="text-white/80 text-sm font-medium truncate">{{
                              cr.title
                            }}</span>
                            <span
                              v-if="cr.shortcut"
                          ...
                          <span v-if="cr.category" class="text-white/20 text-[10px] mt-0.5 block">{{
                            cr.category
                          }}</span>
                        </button>
                        <div
                          v-if="!filteredCanned.length"
            ...
            <Transition name="fade">
              <div
                v-if="showNoteForm"
                class="mt-3 p-3 rounded-xl bg-amber-500/5 border border-amber-500/20"
              >
      ...
      <div class="space-y-4">
          ...
          <dl class="space-y-3">
            ...
            <div v-if="ticket.sentiment" class="flex items-center justify-between">
              ...
              <dd
                ...
                :class="sentimentConfig[ticket.sentiment]?.text"
              >
                ...
                <span
                  ...
                  :class="{
                    'bg-red-500/10 border-red-500/20 text-red-400 animate-pulse':
                      slaCountdown.severity === 'breached' || slaCountdown.severity === 'critical',
                    'bg-amber-500/10 border-amber-500/20 text-amber-400':
                      slaCountdown.severity === 'warning',
                    'bg-green-500/10 border-green-500/20 text-green-400':
                      slaCountdown.severity === 'none',
                  }"
                >
            ...
            <div v-else-if="slaStatus" class="flex items-center justify-between">
              ...
              <dd>
                <span
                  ...
                  :class="slaStatus.bgClass + ' ' + slaStatus.class"
                >
                  {{ slaStatus.label }}
                </span>
              </dd>
        ...
        <UiGlassCard padding="md">
          <h3 class="text-white/60 text-xs font-medium uppercase tracking-wider mb-3">
            Assigned Agent
          </h3>
          <div v-if="ticket.assignedTo" class="flex items-center gap-3">
            <div
              class="w-9 h-9 rounded-full bg-gradient-to-br from-indigo-400 to-purple-500 flex items-center justify-center text-white font-bold"
            >
          ...
          <div v-else class="text-white/40 text-sm text-center py-3">Unassigned</div>
        </UiGlassCard>
        ...
        <UiGlassCard padding="md">
          ...
          <div class="flex flex-wrap gap-1.5 mb-2">
            ...
            >
              {{ tag.name }}
              <button
                @click="removeTagFromTicket(tag.id)"
        ...
        <UiGlassCard padding="md">
          <h3
            class="text-white/60 text-xs font-medium uppercase tracking-wider mb-3 flex items-center gap-1.5"
          >
            <svg
          ...
          <p v-if="analysisError" class="text-red-400 text-xs mt-2 text-center">
            {{ analysisError }}
          </p>
        ...
        <UiGlassCard v-if="ticket.summary" padding="md">
          <h3
            class="text-white/60 text-xs font-medium uppercase tracking-wider mb-2 flex items-center gap-1.5"
          >
            <svg
              class="w-3.5 h-3.5 text-purple-400"
              fill="none"
