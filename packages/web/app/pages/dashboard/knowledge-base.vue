...
<template>
  <div class="animate-fade-in space-y-6">
    ...
    <div class="flex items-center justify-between flex-wrap gap-3">
      ...
      <button v-if="isAdmin" @click="openNewArticle()" class="btn-primary text-sm flex items-center gap-1.5">
    ...
    <div v-else-if="filteredArticles.length === 0" class="glass-card p-12 text-center">
      ...
      <button v-if="isAdmin" @click="openNewArticle()" class="btn-primary mt-4 text-sm">Create your first article</button>
    </div>
    ...
    <div v-else class="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
      ...
      >
        ...
        <div class="flex items-center gap-2 flex-wrap">
          ...
          <span v-if="article.category" class="px-2 py-0.5 rounded-full text-[10px] font-medium bg-indigo-500/10 text-indigo-300 uppercase tracking-wider">
            {{ article.category }}
          </span>
        </div>

        <!-- Title -->
        <h3 class="text-white font-semibold text-sm leading-snug line-clamp-2 group-hover:text-indigo-300 transition-colors">
          {{ article.title }}
        </h3>
    ...
    <Teleport to="body">
      <Transition name="fade">
        <div v-if="showEditor" class="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <!-- Modal -->
          <div class="relative w-full max-w-3xl max-h-[90vh] overflow-y-auto glass-card p-6 space-y-5">
            ...
            <!-- Error -->
            <div v-if="editorError" class="p-3 rounded-lg bg-red-500/10 border border-red-500/30 text-red-300 text-sm">
              {{ editorError }}
            </div>

            <!-- Title -->
            <div>
              <label class="block text-white/60 text-xs font-medium mb-1.5 uppercase tracking-wider">Title</label>
              <input
            ...
            <div class="grid grid-cols-2 gap-4">
              <div>
                <label class="block text-white/60 text-xs font-medium mb-1.5 uppercase tracking-wider">Category</label>
                <input
              ...
              <div>
                <label class="block text-white/60 text-xs font-medium mb-1.5 uppercase tracking-wider">Status</label>
            ...
            <!-- Body (Markdown) -->
            <div>
              <label class="block text-white/60 text-xs font-medium mb-1.5 uppercase tracking-wider">Content (Markdown)</label>
              <textarea
