...
<script setup lang="ts">
  ...
  // Read from URL first, fall back to sessionStorage (survives page reloads)
  const _urlToken = String(route.query.invite ?? '')
  const _storedToken = import.meta.client
    ? (sessionStorage.getItem('auradesk_invite_token') ?? '')
    : ''
  const inviteToken = computed(() => _urlToken || _storedToken)
  ...
  // Persist invite token in sessionStorage so page reloads don't lose it
  if (import.meta.client && inviteToken.value) {
    sessionStorage.setItem('auradesk_invite_token', inviteToken.value)
  }

  // If there's an invite token, validate it
  if (inviteToken.value) {
    try {
      const res = await $fetch<{ data: typeof inviteData.value }>(
        `/api/auth/invite?token=${inviteToken.value}`,
      )
      inviteData.value = res.data
    } catch (e: any) {
      inviteError.value = e?.data?.message ?? 'Invalid invitation link'
      // Clear invalid token from storage
      if (import.meta.client) sessionStorage.removeItem('auradesk_invite_token')
    }
  }
  ...
  async function handleSignup() {
    ...
    try {
      // STEP 0: Sign out any existing session to prevent session conflicts
      // (e.g., admin invites agent while logged in — the admin's session cookie
      //  would persist and the new user would see the admin's profile)
      ...
      // STEP 3: Force a fresh sign-in to ensure the correct session is active
      // This prevents the "wrong user name" bug when the browser had a previous session
      const { error: signInError } = await supabase.auth.signInWithPassword({
        email: result.data.email,
        password: password.value,
      })
      if (signInError) {
        console.warn('[signup] Post-signup sign-in failed:', signInError.message)
      }

      // Clear the invite token from sessionStorage after successful signup
      if (import.meta.client) sessionStorage.removeItem('auradesk_invite_token')
  ...
  <div>
    ...
    <div class="mb-8">
      ...
      <p class="text-white/50 text-sm mt-2">
        {{
          inviteData
            ? `You've been invited to join ${inviteData.orgName}`
    ...
    >
      <div class="flex items-center gap-3">
        <div
          class="w-10 h-10 rounded-xl bg-indigo-500/20 flex items-center justify-center flex-shrink-0"
        >
          <svg
        ...
        <div>
          ...
          <p class="text-white/40 text-xs">
            Invited by {{ inviteData.invitedBy }} as {{ inviteData.role }}
          </p>
        </div>
      </div>
    </div>
    ...
    <form v-else class="space-y-5" @submit.prevent="handleSignup">
      ...
      <button
        type="submit"
        :disabled="
          isLoading || !email || !password || !fullName || (!inviteData && !organizationName)
        "
