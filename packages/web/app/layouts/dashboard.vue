...
<script setup lang="ts">
  ...
  // Always fetch fresh user data on dashboard load to ensure correct session
  // This prevents stale profile data after signup/login/invite flows
  // (Siempre obtener datos frescos del usuario al cargar el dashboard
  //  para asegurar la sesión correcta después de signup/login/invite)
  ...
  async function fetchNotifications() {
    ...
    try {
      const res = await $fetch<{ data: { notifications: Notification[]; unreadCount: number } }>(
        '/api/notifications',
      )
...
<template>
  <div class="flex h-screen bg-app-gradient overflow-hidden">
    ...
    >
      <div
        class="flex flex-col h-full bg-[#0d0b24]/90 backdrop-blur-xl border-r border-white/[0.06]"
      >
        ...
        <div class="flex items-center gap-3 p-6 border-b border-white/10">
          ...
          <div class="min-w-0">
            ...
            <p class="text-xs" :class="planColors[authStore.plan]">{{ authStore.plan }} plan</p>
          </div>
        </div>
        ...
        <div class="p-4 border-t border-white/10">
          ...
          <a
            :href="`/portal${authStore.organization?.slug ? '?org=' + authStore.organization.slug : ''}`"
            target="_blank"
            class="flex items-center gap-3 px-3 py-2 rounded-xl text-sm font-medium text-white/50 hover:bg-white/5 hover:text-white transition-all duration-200 mb-2 group"
          >
            <svg
              class="w-5 h-5 flex-shrink-0 text-white/30 group-hover:text-white/60 transition-colors"
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
            >
            ...
            Customer Portal
            <svg
              class="w-3 h-3 ml-auto text-white/20"
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
            >
          ...
          <div class="flex items-center gap-3 px-3 py-2">
            ...
            <div
              class="w-8 h-8 rounded-full bg-gradient-to-br from-indigo-400 to-purple-500 flex items-center justify-center text-white text-sm font-bold flex-shrink-0"
            >
    ...
    <div class="flex-1 flex flex-col min-w-0 overflow-hidden">
      ...
      <header
        class="relative z-40 flex items-center gap-4 px-6 py-4 border-b border-white/[0.06] bg-white/[0.02] backdrop-blur-sm"
      >
            ...
            <Transition name="fade">
              ...
              >
                ...
                <div v-if="notifLoading" class="flex items-center justify-center py-8">
                  <div
                    class="animate-spin w-5 h-5 border-2 border-indigo-500 border-t-transparent rounded-full"
                  />
                </div>
                ...
                <div
                  ...
                  class="overflow-y-auto max-h-[22rem] divide-y divide-white/[0.04]"
                >
                  <NuxtLink
                ...
                <div v-else class="py-8 text-center">
                  <svg
                    class="w-8 h-8 mx-auto text-white/10 mb-2"
                    fill="none"
                    stroke="currentColor"
                    viewBox="0 0 24 24"
                  >
